...
public class NewsRepository {
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                        ...
                        News news = new News(current.getInt("id"),
                                current.getString("categoryName"), current.getString("title"),current.getString("text"), LocalDateTime.parse(current.getString("date"), formatter), current.getString("image"));
                        data.add(news);
                    }
                }
        ...
        srv.execute(()->{
            try {
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                    DateTimeFormatter formatter = DateTimeFormatter.ISO_OFFSET_DATE_TIME;
                    News news = new News(current.getInt("id"),
                            current.getString("categoryName"), current.getString("title"),current.getString("text"),
                            LocalDateTime.parse(current.getString("date"), formatter), current.getString("image"));
                    msg.obj = news;
                }
    ...
    public void postComment(ExecutorService srv, Handler uiHandler, String name, String comment, String news_id){
        srv.execute(()->{
